...
export interface StandardTableProps<T> extends Omit<TableProps<T>, 'columns'> {
  ...
  data: {
    movies: MovieItem[];
    page: StandardTableProps<MovieItem>['pagination'];
  };
  selectedRows: MovieItem[];
  onSelectRow: (rows: any) => void;
}

export interface StandardTableColumnProps extends ColumnProps<MovieItem> {
  needTotal?: boolean;
  total?: number;
}
...
class StandardTable extends Component<StandardTableProps<MovieItem>, StandardTableState> {
  ...
  constructor(props: StandardTableProps<MovieItem>) {
    super(props);
    const { columns } = props;
  ...
  handleRowSelectChange: TableRowSelection<MovieItem>['onChange'] = (
    selectedRowKeys,
    selectedRows: MovieItem[],
  ) => {
  ...
  handleTableChange: TableProps<MovieItem>['onChange'] = (pagination, filters, sorter, ...rest) => {
                ...
                {needTotalList.map((item, index) => (
                  <span style={{ marginLeft: 8 }} key={item.dataIndex}>
                    ...
                    <span style={{ fontWeight: 600 }}>
                      {item.render ? item.render(item.total, item as MovieItem, index) : item.total}
                    </span>
                  </span>
                ))}
